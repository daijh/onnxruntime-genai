...
Z-Image-Turbo pipeline (see zimage.py), in either float16 or float32.

These mirror the tiny helper graphs the WebNN JS demo generates to keep intermediate tensors
GPU-resident, but shaped to match our self-built transformer's `[1, 16, H, W]`
...
# Entry point
# ======================================================================
def build_helper_models(output_dir, precision, height=512, width=512, num_inference_steps=8):
    """Build scheduler_step_model / vae_pre_process_model / sc_prep_model into output_dir.

    Args:
        output_dir: directory to write `<name>_model_<precision>.onnx` into.
        precision: exactly "f16" or "f32" -- run this twice for both, same as how the
            transformer/text_encoder builders are invoked once per precision.
        height, width: pixel resolution used only to size the internal verification tensors;
            the graphs themselves keep height/width dynamic.
        num_inference_steps: used only to size the scheduler_step verification tensors.
    """
    ...
    print(f"output_dir={output_dir} precision={precision}")
    print("=" * 60)
    ...
    build_one(
        ...
        output_dir, precision, verify_scheduler_step,
    )
    ...
    build_one(
        ...
        output_dir, precision, verify_vae_pre_process,
    )
    ...
    build_one(
        ...
        output_dir, precision, verify_sc_prep,
    )

    print("\n" + "=" * 60)
    print(f"Done! Models saved to {os.path.abspath(output_dir)}/")
    print("=" * 60)
